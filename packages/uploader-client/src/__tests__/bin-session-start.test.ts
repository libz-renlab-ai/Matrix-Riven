/**
 * 薄壳 bin-session-start 最小测试（设计文档 §7）。
 *
 * 覆盖：
...
import { main } from '../bin-session-start.js';
...
describe('bin-session-start main()', () => {
  ...
  it('正常输入：emitCcStatus 被调用一次，event/sessionId/cwd 正确', async () => {
    const stdinReader = async () =>
      JSON.stringify({ session_id: 'ses-abc', cwd: '/home/user/proj' });
  ...
  it('TEAMAGENT_DISABLED=1 → emitCcStatus 不被调用', async () => {
    ...
    const stdinReader = async () =>
      JSON.stringify({ session_id: 'ses-disabled', cwd: '/home/user/proj' });
