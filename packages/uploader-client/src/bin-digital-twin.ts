...
// Auto-invoke when this file is the entry point. Use process.argv[1] —
// works in both ESM (vitest) and CJS (tsup-bundled) contexts.
// Explicitly exclude 'bin-digital-twin-tap' to avoid substring collision.
const _argv1Base = basename(process.argv[1] ?? '');
if (
  _argv1Base.startsWith('bin-digital-twin') &&
  !_argv1Base.startsWith('bin-digital-twin-tap')
) {
  void (async () => {
    let parsed: DigitalTwinParsedArgs;
    try {
      parsed = parseDigitalTwinArgs(process.argv.slice(2));
    } catch (err) {
    ...
    try {
      result = await executeDigitalTwin(parsed);
    } catch (err) {
      process.stderr.write(String(err) + '\n');
      process.exit(1);
    }
